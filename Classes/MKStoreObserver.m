	...
	for (SKPaymentTransaction *transaction in transactions)
	{
		switch (transaction.transactionState)
		{
			case SKPaymentTransactionStatePurchased:
                ...
                //вот тут нужно ломиться на сервер, слать ему transaction.receipt и ждать в ответ дату, до которой аккаунт в Эльбе будет продлён
                //а также запомнить её и писать где-то в уголочке, обновляя после каждой следующей покупки. Это может быть текст
                //в кнопке, отвечающей за проверку покупок. По этой же кнопке можно перепроверять дату, до которой оплачен аккаунт.
            {
