...
- (NSArray *)filters
{
	return [NSArray arrayWithObjects:
			...
			DATE_FORMAT, GET_ITEM, COLOR_FORMAT, 
			nil];
}
...
- (NSObject *)filterInvoked:(NSString *)filter withArguments:(NSArray *)args onValue:(NSObject *)value
{
	...
	else if ([filter isEqualToString:GET_ITEM]) {
		
		if ([value isKindOfClass:[NSArray class]] && ([args count] == 1))
		{
			return @"GOOD BYE!";
			//return [value objectAtIndex:[args objectAtIndex: 0]];
		}
		else {
			return @"HELLO!";
		}
