...
- (NSString *) verifyReceipt:(NSData*) receiptData;
...
-(void) requestProductData
{
	SKProductsRequest *request= [[SKProductsRequest alloc] initWithProductIdentifiers:[NSSet setWithObjects: 
                                                                                       kInAppPurchaseIPMonthSubscriptionProductId,
...
- (void) buyFeature:(NSString*) featureId
{
	...
	{
		UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Запрос на обзор одобрен"
														message:@"Вы можете использовать данную функцию для обзора приложения"
													   delegate:self 
	...
	{
		UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Внутренние покупки отключены"
														message:@"Проверьте настройки Родительского контроля и попробуйте ещё раз"
													   delegate:self 
											  cancelButtonTitle:@"Ок"
											  otherButtonTitles: nil];
		[alert show];
            ...
            forReceipt:(NSData*) receiptData
{
    ...
    // it should normally take a couple of seconds on a good 3G connection
          NSString *serverAnswer = [self verifyReceipt:receiptData];
        if([serverAnswer isEqualToString:@"NO"]) 
        {
            return;
        }
        else
        {
            [[NSUserDefaults standardUserDefaults] setValue:[NSString stringWithFormat:@"Веб-сервис Эльба оплачен до: %@", serverAnswer] forKey:@"tillDate"];
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Поздравляем!" message:[NSString stringWithFormat:@"Все платные услуги Эльбы теперь доступны вам до %@.", serverAnswer] delegate:self cancelButtonTitle:@"Ок" otherButtonTitles: nil];
            [alert show];
            [alert release];
    ...
    if([_delegate respondsToSelector:@selector(transactionSuccessful)])
		[_delegate transactionSuccessful];
}
...
- (void) transactionCanceled: (SKPaymentTransaction *)transaction
{
	...
	NSLog(@"User cancelled transaction: %@", [transaction description]);
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Транзакция отклонена iTunes Store" 
                                                        message:@"Операция не удалась. Вы можете повторить попытку позже."
                                                       delegate:self 
                                              cancelButtonTitle:@"Ок"
                                              otherButtonTitles: nil];
        [alert show];
...
-(NSString *) verifyReceipt:(NSData*) receiptData
{
	...
	NSURL *url = [NSURL URLWithString: @"NDA"];
	...
	[theRequest setHTTPBody:[receiptDataString dataUsingEncoding:NSUTF8StringEncoding]];
